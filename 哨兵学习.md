...
## 一，wifi连车(同一个wifi不用每次都走这个流程，不同的则需要)
...
6. 然后再输入ifconfig,会弹出一堆东西，找到wlp87so(总之是w开头的东西，这个是网卡)，然后找到他这一堆东西的inet 192.168.31.18，记住这个
...
2. 如果需要找到哨兵的bag，连上车后cd Documents/ ，然后ls，然后cd sentry/，ls。其中使用**ls  -lh**可以看到每个包的具体大小，尽量把每个超过4G的包都拉到本地，应该是国赛的。
...
1. rm_sentry目录下的map中的**map.yaml**中的意义
...
### 范围重定位（机器人随意拿到一个位置，他可以自己判断自己的位置）



### costmap(代价地图：表示机器人周围环境的网格地图)
...
### **behavior_tree.cpp      main.cpp     run_tree.launch** 三个文件的的作用
